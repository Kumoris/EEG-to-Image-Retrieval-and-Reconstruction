...
echo "=== Channel-Conformer seed=$SEED ===" "$(date)"
python3 -m eeg_cogcappro.train_atms \
    ...
    --seed "$SEED" \
    --output-dir "runs/conformer_ch_vitl_seed${SEED}" \
    --device cuda
...
python3 -m eeg_cogcappro.eval_atms \
    ...
    --feature-cache "$CACHE" \
    --feature-key rn50_feature \
    --ckpt "runs/conformer_ch_vitl_seed${SEED}/best.pt" \
    --split test \
    --tta-n 5 \
    --output "results/conformer_ch_vitl_seed${SEED}_test_tta5.json" \
    --device cuda
